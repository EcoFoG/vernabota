...
We can then run *CompareSim*, visualise the scenarios and their results using *summary* and plot their accuracy using *plot* or *autoplot*.
...
Param <- data.frame(priors = c(1,1,2),  
                    dataAsso = c(2,1,2), 
                    weights = c(0.5,0.5,0.5),
                    eps = c(0.01,0.01,0.01),
                    Determ = c(FALSE,FALSE,FALSE))

VBS_test2 <- CompareSample(NbSamples = 3,Param = Param ,
                       priors = priors, D2fill = Data2fill, DAsso = DAsso,
                       pc2fill = 10, pcFamilyDet = 25, pcGenusDet = 25, 
                       NbSim = 10, Results_Simulations = FALSE)
...
library(ggplot2)
ggplot(aes(y = accuracy, x = scenario, fill = sample), data = VBS_test2[[2]]) + geom_boxplot()
```

Here, the ranking is preserved between different test datasets, confirming that the variability between scenarios is higher than the one between samplings. We can then choose a scenario according to this ranking. 
